...
import { Download, FileSpreadsheet, BarChart3, Clock, Users, Archive } from "lucide-react";
...
type Nurse = { id: string; name: string; role: string; ward: string | null; facility: string | null; hours_this_month: number; target_hours: number };
type ShiftLog = { nurse_id: string; shift_date: string; shift_type: string; started_at: string; ended_at: string | null; hours_logged: number | null; period_start: string };
type PeriodHours = { nurse_id: string; period_start: string; period_end: string; total_hours: number; total_shifts: number };
...
function ReportsPage() {
  ...
  const [tab, setTab] = useState<"overview" | "hours" | "periods">("overview");
  ...
  async function closePeriod() {
    if (!confirm("Close the current period? This will save all nurses' hours to the period archive and reset their monthly hour counter to 0.")) return;
    ...
    try {
      ...
      for (const nurse of nurses) {
        ...
        await supabase.from("nurse_period_hours").upsert(
          { nurse_id: nurse.id, period_start: periodStart, period_end: today, total_hours: totalHours, total_shifts: totalShifts },
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 bg-muted/50 rounded-lg p-1 mb-6 w-fit">
        ...
        <button type="button" className={tabCls("hours")} onClick={() => setTab("hours")}>Shift Hours</button>
        <button type="button" className={tabCls("periods")} onClick={() => setTab("periods")}>Period Archive</button>
      </div>
        ...
        <>
          ...
          <div className="bg-card border rounded-xl p-5 shadow-soft">
            ...
            {activeNurses.length === 0 ? (
              <EmptyState icon={<Clock className="h-6 w-6" />} title="No hours logged yet" description="Hours appear here as nurses start and complete their shifts." />
            ) : (
              <div className="space-y-2">
                {nurses.filter((n) => nurseHoursMap.has(n.id)).map((n) => {
                  ...
                  return (
                    <div key={n.id} className="flex items-center gap-3 text-sm">
                      ...
                      <div className="flex-1 h-5 rounded-full bg-muted overflow-hidden">
                        <Progress value={Math.min(pct, 100)} className="h-full rounded-full bg-primary/70" />
                      </div>
      ...
      {tab === "hours" && (
        <div className="space-y-4">
          <div className="flex justify-end gap-2">
            <button type="button" onClick={exportCurrentHours} className="inline-flex items-center gap-2 h-9 px-3 rounded-md border bg-card text-sm hover:bg-muted">
              <FileSpreadsheet className="h-4 w-4 text-emerald-600" /> Summary Excel
            </button>
            <button type="button" onClick={exportDetailedLogs} className="inline-flex items-center gap-2 h-9 px-3 rounded-md border bg-card text-sm hover:bg-muted">
              <Download className="h-4 w-4" /> Detailed Logs
            </button>
          </div>

          {shiftLogs.length === 0 ? (
            <EmptyState icon={<Clock className="h-6 w-6" />} title="No shift logs" description="Shift logs appear here once nurses start tracking their shifts." />
          ) : (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {shiftLogs.map((log) => {
                    ...
                    return (
                      <tr key={`${log.nurse_id}-${log.shift_date}`} className="border-t hover:bg-muted/30">
                        <td className="px-4 py-3 font-medium">{nurse?.name ?? "—"}</td>
                        <td className="px-4 py-3 text-muted-foreground">{log.shift_date}</td>
                        <td className="px-4 py-3">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${log.shift_type === "M" ? "bg-amber-100 text-amber-700" : "bg-indigo-100 text-indigo-700"}`}>
                            {log.shift_type === "M" ? "Morning" : "Night"}
                          </span>
                        </td>
                        <td className="px-4 py-3 tabular-nums text-muted-foreground">
                          {log.started_at ? new Date(log.started_at).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" }) : "—"}
                        </td>
                        <td className="px-4 py-3 tabular-nums text-muted-foreground">
                          {log.ended_at ? new Date(log.ended_at).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" }) : <span className="text-emerald-600 text-xs font-medium">Running</span>}
                        </td>
                        <td className="px-4 py-3 text-right tabular-nums font-medium">
                          {log.hours_logged != null ? `${Number(log.hours_logged).toFixed(2)}h` : "—"}
                        </td>
                      </tr>
                    );
                  })}
      ...
      {tab === "periods" && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <button type="button" onClick={exportPeriodArchive} className="inline-flex items-center gap-2 h-9 px-3 rounded-md border bg-card text-sm hover:bg-muted">
              <FileSpreadsheet className="h-4 w-4 text-emerald-600" /> Export Archive
            </button>
          </div>

          {periodSummaries.length === 0 ? (
            <EmptyState icon={<Archive className="h-6 w-6" />} title="No archived periods" description='Use "Close Period" at the end of each 28-day cycle to save a snapshot here.' />
          ) : (
            <div className="bg-card border rounded-xl shadow-soft overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-muted/50 text-xs uppercase tracking-wide text-muted-foreground">
